...
class Page:
    ...
    def wait_and_click(self, *locator: object) -> object:
        self.wait.until(
            EC.element_to_be_clickable(locator),
            message=f'Element by locator {locator} not clickable'
        ).click()
    ...
    def verify_text(self, expected_text: object, *locator: object) -> object:
        actual_text = self.driver.find_element(*locator).text
        assert actual_text == expected_text, f'Expected {expected_text} did not match actual {actual_text}'

    def verify_partial_text(self, expected_partial_text: object, *locator: object) -> object:
        actual_text = self.driver.find_element(*locator).text
        assert expected_partial_text in actual_text.lower(), f'Expected {expected_partial_text} not in actual {actual_text.lower()}'
